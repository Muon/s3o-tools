...
class S3O(object):
    def __init__(self, data):
        ...
        magic, version, radius, height, mid_x, mid_y, mid_z, root_piece_offset, \
        collision_data_offset, tex1_offset, tex2_offset = header
...
class S3OPiece(object):
    def __init__(self, data, offset, parent=None):
        ...
        name_offset, num_children, children_offset, num_vertices, vertex_offset, \
        vertex_type, primitive_type, num_indices, index_offset, \
        collision_data_offset, x_offset, y_offset, z_offset = piece
        
        #print(piece)
        
        self.parent = parent
        self.name = _get_null_terminated_string(data, name_offset)
        self.primitive_type = ["triangles", "triangle strips", "quads"][primitive_type]
        ...
        for i in range(num_children):
            current_offset = children_offset + _S3OChildOffset_struct.size * i
            child_offset, = _S3OChildOffset_struct.unpack_from(data, current_offset)
    ...
    def serialize(self, offset):
        ...
        child_data = b''
        for i in range(len(self.children)): # make an empty buffer to put size in later
